...
const addProduct = async (req, res) => {
  try {
    const { name, rate, stocks, category, kilogramOption, image, shelfLifeDays } = req.body;
...
const updateProduct = async (req, res) => {
  try {
    const { productId } = req.params;
    const { name, rate, kilogramOption, category, stocks, image, shelfLifeDays } = req.body;
    if (productId) {
      const isProductExit = await productModel.findById(productId);
      if (image !== "") {
        const result = await coludinary.v2.uploader.upload(image, {
        ...
        isProductExit.shelfLifeDays = shelfLifeDays;
